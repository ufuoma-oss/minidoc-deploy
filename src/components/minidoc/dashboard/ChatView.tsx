...
import { ArrowUp, SquarePen, Menu, Globe, Plus } from 'lucide-react';
...
const ChatView: React.FC<ChatViewProps> = ({
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      {/* Fixed Bottom Input - Clean, glued to bottom with curved edges */}
      <div className="fixed bottom-0 left-0 right-0 md:left-72 z-30">
        {/* Single clean container - no inner box, completely flat */}
        <div className="mx-2 mb-2 rounded-2xl bg-neutral-100 flex items-end px-3 py-2.5 gap-2">
          {/* Plus icon at far left - only shows when NOT typing */}
          {inputText.trim() === '' && (
            <motion.button 
              ...
              whileTap={{ scale: 0.95 }}
              className="p-1 text-neutral-500 hover:text-black flex-shrink-0 mb-0.5"
              title="Add files"
            >
          ...
          {/* Text input area - auto expands as text grows */}
          <textarea
            ref={textareaRef}
            value={inputText}
          ...
          {/* Right side icons - ALL hide when typing, only send remains */}
          <div className="flex items-center flex-shrink-0 gap-3 mb-0.5">
            {/* Globe and Voice - only show when NOT typing */}
            {inputText.trim() === '' && (
              <motion.div 
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
